...
- **Multi-hop delegation chains.** A `Grant` is single-hop: one issuer, one subject, no representation of "A delegated to B, who sub-delegated a narrower scope to C." Microsoft's Agent Governance Toolkit's `AgentMesh Identity and Trust` spec (`docs/specs/AGENTMESH-IDENTITY-TRUST-1.0.md`, checked live — see `agent-guard/docs/DESIGN.md`'s citation for the caveat that this is one individual's project hosted under the `microsoft` org, not a centrally-built Microsoft product) states the property a chained version would need to hold: "Scope chains only narrow. Delegation can never grant more capabilities than the parent holds," enforced via hash-chained delegation records. Worth citing as the shape of the problem if multi-hop delegation becomes a real V2 need — not built here, and not a small increment on `Grant`/`verify()`; monotonic-narrowing enforcement across a chain is a genuinely different verification problem than this project's single issuer-to-subject check.
